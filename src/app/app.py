...
from .monotic_decreasing_functions import monotic_decreasing_function_factory, search_monotic_decreasing_function_subclasses, search_normalization_subclasses, normalization_factory
import time
...
def main(parser=argparse.ArgumentParser(prog="genset", description="Non-dominated set generator")):
    ...
    parser.add_argument(
        "-p", "--points", type=int, required=False, help="you shall generate a number of points", default=50
    )
    parser.add_argument(
        "-m", "--function", 
        required=False, 
    ...
    parser.add_argument(
        "-n", "--normalize", help="Normalize space", default="Null", required=False,
        choices=search_normalization_subclasses()
    )
    parser.add_argument(
    ...
    pareto_front_generator = ParetoFrontGenerator(
            function=monotic_decreasing_function_factory(args.function, args.dimension, args.points, normalization_factory(args.normalize), args.alpha),
